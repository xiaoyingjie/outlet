<template>
  <div class="treeWraps" style="height: 100%" v-loading.treeWraps="loading">
    ...
    <div id="rMenu">
      <ul>
        <li v-if="false" id="m_add" @click="addTreeNode(1)">添加同级</li>
  ...
  export default {
    ...
    methods: {
      ...
      initzTree (zNodes) {
        ...
        this.$emit('selectDept', {c: zNodes[0].c, n: zNodes[0].n, id: zNodes[0].id})
        this.loading = false
      },
      zTreebeforeClick (treeId, treeNode) {
        ...
        if (treeObj.isSelectedNode(treeNode)) {
          ...
          this.$emit('selectDept', {c: '', n: '', id: ''})
          return false
        } else {
          this.$set(treeNode, 'uc', treeNode.c)
          this.$emit('selectDept', {c: treeNode.uc, n: treeNode.n, id: treeNode.id})
